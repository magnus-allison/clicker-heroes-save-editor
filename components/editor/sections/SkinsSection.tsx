...
export const SkinsSection = ({ defaultOpen, showToast }: Props) => {
	...
	const selectedSkinId = Number(getValueAtPath(saveData, ["currentAutoclickerSkin"]) ?? autoClickerSkins[0].id);
	...
	return (
		...
		>
			<div className="space-y-6">
				<div>
					<p className="text-[11px] uppercase tracking-[0.08em] text-(--color-text-dim)">Current skin</p>
						...
						{autoClickerSkins.map((skin) => {
							...
							return (
								<button
									...
									type="button"
								>
									<EditorImage alt={skin.name} className="mx-auto h-16 w-16 object-contain" size={64} src={skin.imageSrc} />
									<p className="mt-3 text-center text-[12px] text-(--color-text-secondary)">{skin.name}</p>
								</button>
							);
						})}
					</div>
				...
				<div className="flex items-center justify-between gap-3">
					<p className="text-[11px] uppercase tracking-[0.08em] text-(--color-text-dim)">Unlocked skins</p>
					<Button
					...
					<div className="overflow-x-auto">
						<table className="min-w-full border-collapse text-left text-[13px] text-(--color-text-secondary)">
							...
							<tbody className="bg-(--color-bg)">
								{autoClickerSkins.map((skin) => (
									<tr className="not-last:border-b not-last:border-(--color-border-subtle)" key={skin.id}>
										<td className="px-4 py-3">
											<EditorImage alt={skin.name} className="h-11 w-11 object-contain" size={44} src={skin.imageSrc} />
										</td>
