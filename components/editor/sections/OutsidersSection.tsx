...
const formatOutsiderMetric = (value: number) => {
	...
	return value.toExponential(2).replace("+", "").replace(/\.?0+(e\d+)$/, "$1");
};
							...
							<table className="min-w-full border-collapse text-left text-[13px] text-(--color-text-secondary)">
								<tbody className="bg-(--color-bg)">
									{ancientSoulFields.map((field) => (
										<tr className="not-last:border-b not-last:border-(--color-border-subtle)" key={field.path.join(".")}>
											<td className="px-4 py-3 text-(--color-text)">{field.label}</td>
											<td className="min-w-55 px-4 py-3">
												<BoundFieldControl kind={field.kind} path={field.path} selectOnFocus />
											</td>
										</tr>
									))}
								</tbody>
						...
						<table className="min-w-full border-collapse text-left text-[13px] text-(--color-text-secondary)">
							...
							<tbody className="bg-(--color-bg)">
								{outsiderRows.map(({ field, level, spent, stats }) => (
									<tr className="not-last:border-b not-last:border-(--color-border-subtle)" key={field.id}>
										<td className="px-4 py-3 align-top">
											<EditorImage alt={field.name} className="h-12 w-12 object-contain" size={48} src={field.imageSrc} />
										</td>
										<td className="px-4 py-3 align-top">
											...
											{field.capDescription && stats.secondary !== undefined ? (
												<p className="mt-1 leading-6 text-(--color-text-secondary)">
													{field.capLabel}
													{stats.secondary.toFixed(2).replace(/\.?0+$/, "")} {field.capDescription}
												</p>
											) : null}
										</td>
										...
										<td className="px-4 py-3 align-top text-(--color-text)">{stats.spent.toLocaleString("en-US")} AS</td>
										<td className="min-w-40 px-4 py-3 align-top">
											<NumberInput
												disabled={!saveData}
