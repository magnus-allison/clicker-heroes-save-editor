					...
					<table className='min-w-full border-collapse text-left text-[13px] text-(--color-text-secondary)'>
						...
						<tbody className='bg-(--color-bg)'>
							{orderedHeroes.map((hero) => {
								...
								const imageSrc =
									epicLevel >= 1 && hero.gildedImage ? hero.gildedImage : hero.image;

								return (
									<tr
										className={
											epicLevel >= 1
												? 'bg-(--color-gilded-bg) text-(--color-gold-text) not-last:border-b not-last:border-(--color-gilded-border)'
												: 'not-last:border-b not-last:border-(--color-border-subtle)'
										}
										key={hero.heroId}
									>
										...
										<td className='px-4 py-3'>
											...
											{epicLevel >= 1 ? (
												<p className='mt-1 text-[11px] uppercase tracking-[0.08em] text-(--color-gold-text)'>
